...
CONFIGS = {
    'univariate': {
        'FEATURES': 'S',
        'TARGET': 'OT', 
        'BASE_DESCRIPTION': 'Univariate LSTM Regression on Electricity Data (Pred Len: {})',
        'INPUT_SIZE': 1 # For univariate, input size is 1 feature
    },
    'multivariate': {
        'FEATURES': 'M',
        'TARGET': None, # Target is determined by FEATURES='M' in load function
        'BASE_DESCRIPTION': 'Multivariate LSTM Regression on Electricity Data (Pred Len: {})',
        'INPUT_SIZE': None
    }
}
...
class ElectricityDataset(Dataset):
    def __init__(self, root_path, data_path, flag='train', size=None,
        ...
        self.external_scaler = scaler # Accept external scaler
        self.__read_data__()
    ...
    def __read_data__(self):
        ...
        if self.scale:
            if self.external_scaler is not None:
                self.scaler = self.external_scaler # Use external scaler if provided
                data = self.scaler.transform(df_data.values)
            elif self.set_type == 0: # If train set and no external scaler, fit here
    ...
    def __getitem__(self, index):
        ...
        seq_y_full_slice = self.data_y[s_end : s_end + self.pred_len] 


        # Select output features based on FEATURES flag
        if self.features == 'S':
             seq_y = seq_y_full_slice[:, 0:1] 
        elif self.features == 'M':
             seq_y = seq_y_full_slice 
        else:
...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_features, seq_len, pred_len):
        ...
        # Linear layer to map LSTM output to the desired prediction dimension
...
if __name__ == "__main__":
    ...
    for hidden_size in hidden_sizes_to_test:
        for learning_rate in learning_rates_to_test:
            ...
            model_tune = LSTMModel(INPUT_SIZE_TUNING, hidden_size, 2, OUTPUT_FEATURES_TUNING, SEQ_LEN, TUNING_PRED_LEN).to(device) # Assuming 2 layers fixed for tuning
            ...
            if val_mse < best_val_mse:
                ...
                best_hyperparameters = {'HIDDEN_SIZE': hidden_size, 'LEARNING_RATE': learning_rate, 'NUM_LAYERS': 2} 
    ...
    # Now iterate through all prediction lengths and train/test with best HPs
    all_results = {} # Reset results dictionary for final testing
    ...
    for current_pred_len in PRED_LENS_TO_TEST:
        ...
        # Note: Output size depends on the current PRED_LEN and Output Features
        ...
        model = LSTMModel(INPUT_SIZE_RUN, BEST_HIDDEN_SIZE, BEST_NUM_LAYERS, OUTPUT_FEATURES_RUN, SEQ_LEN, PRED_LEN).to(device)
        criterion = nn.MSELoss()
        ...
        # --- Testing ---
        print("\nStarting testing...")
        # Corrected unpack: receive 3 values from evaluate
